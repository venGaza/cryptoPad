/*
 * Description: This program pairs with the decrypt-daemon program and functions as the client
 * in the client / server communication model. This program takes a checks validity of the
 * command input and then sends it over to the file to be decrypted. Once it gets the
 * message back from the server, it prints the decrypted message to stdout.
...
int main(int argc, const char * argv[]) {
    ...
    sprintf(command, "%d,dec", dataPortNum);
    ...
    //Wait for decrypted message from server then output once received
    charRead = recv(dataSocketFD, buffer, 99999, 0);
    if (charRead < 0) {
        error("Error: read()");
    }
    
    //Add newline back on to decrypted message and print it out
